...
const heroSlides = [
  ...
  { id: 2, src: "https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=1400&q=80&auto=format&fit=crop", alt: "Colleagues reviewing data on a large monitor" },
  { id: 3, src: "https://images.unsplash.com/photo-1523240795612-9a054b0db644?w=1400&q=80&auto=format&fit=crop", alt: "Graduate holding a completion certificate" },
];
    ...
    <div className="absolute inset-0 overflow-hidden">
      {heroSlides.map((slide, i) => (
        <div key={slide.id} className="absolute inset-0" style={{ opacity: i === current ? 1 : 0, transition: "opacity 900ms ease-in-out", zIndex: i === current ? 2 : 1 }}>
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#0a1a3c]/85 via-[#0a1a3c]/55 to-[#0a1a3c]/20" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#0a1a3c]/70 via-transparent to-transparent" />
        </div>
      ))}
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-5 md:px-10 flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link href="/auth/login" className="text-sm text-gray-600 hover:text-gray-900 font-medium px-5 py-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 transition-colors">Log in</Link>
            <Link href="/auth/register" className="inline-flex items-center gap-1.5 text-sm bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-xl transition-all shadow-md shadow-blue-200">
              Get Started
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${menuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="border-t border-gray-100 bg-white px-5 py-4 flex flex-col gap-1">
            ...
            <div className="flex gap-3 pt-3 mt-1 border-t border-gray-100">
              <Link href="/auth/login" onClick={() => setMenuOpen(false)} className="flex-1 text-center text-sm border border-gray-200 text-gray-700 font-medium py-2.5 rounded-xl hover:border-blue-300 hover:text-blue-600 transition-all">Log in</Link>
      ...
      <section className="relative min-h-[92vh] flex items-center pt-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 py-20 w-full">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-white/10 border border-white/25 text-white text-xs font-semibold px-4 py-2 rounded-full mb-7 backdrop-blur-sm tracking-wide">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link href="/contact" className="inline-flex items-center justify-center gap-2 bg-transparent hover:bg-white/10 border border-white/40 text-white font-semibold px-7 py-3.5 rounded-xl text-sm transition-all backdrop-blur-sm">
                Contact Us
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6"/></svg>
              </Link>
            </div>
        ...
        {/* ── STATS STRIP (overlaid on hero, matches screenshot) ── */}
        <div className="absolute bottom-0 left-0 right-0 z-10">
          <div className="max-w-7xl mx-auto px-6 md:px-12">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 pb-8">
              {stats.map((s) => (
                <div key={s.label} className="flex items-center gap-3 bg-white/10 border border-white/20 backdrop-blur-md rounded-2xl px-4 py-3.5">
                  <svg className="w-5 h-5 text-blue-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                  <div>
                    <p className="text-lg md:text-xl font-black text-white leading-none">{s.value}</p>
                    <p className="text-[11px] md:text-xs text-blue-100 mt-1">{s.label}</p>
                  </div>
                </div>
              ))}
            </div>
