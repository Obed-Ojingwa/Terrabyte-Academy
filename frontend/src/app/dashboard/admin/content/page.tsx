...
export default function AdminContentPage() {
  ...
  const createPost = useMutation({
    mutationFn: async () => api.post("/blog", { title: "New post", slug: `post-${Date.now()}`, excerpt: "Draft", is_published: false }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-blog"] }); toast.success("Blog draft created"); },
    onError: () => toast.error("Unable to create blog draft")
  });

  const createEvent = useMutation({
    mutationFn: async () => api.post("/events", { title: "New event", start_date: new Date().toISOString(), is_online: true }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-events"] }); toast.success("Event created"); },
      ...
      {tab === "blog" && (
        <div className="bg-[#071428] border border-white/[0.06] rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => createPost.mutate()} className="rounded-xl bg-brand-500 px-4 py-2 text-sm">Create draft</button>
          </div>
